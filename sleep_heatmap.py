...
def analyze(files):
    ...
    for prefix_name in files.keys():
        ...
        genotypes = utils.load_genotypes(
            group["genotypes"], group["fish_used"], "down"
        )
...
for prefix, data in main_data["run"].items():
    save_whole_plot(data["data"].copy(), data["mask"], prefix, os.path.join("data", "sleep", "output", prefix))
    for arena in data["arenas"]:
        save_arena_plot(data["data"].copy(), arena, f"{prefix}: Arena {arena}", os.path.join("data", "sleep", "output", f"{prefix}_{arena}"))
for genotype, prefixes in main_data["by_genotype"].items():
    for prefix, data in prefixes.items():
        save_whole_plot(data["data"].copy(), data["mask"], f"{prefix}: {genotype}", os.path.join("data", "sleep", "output", f"{prefix}_{genotype}"))
