...
all_files = utils.find_heatmap_data("mirror_biting", suffixes)

# get files
main_files = all_files["main_files"]
wildtype_files = all_files["wildtype_files"]
arena_map = utils.load_arena_map(
    os.path.join("data", "arenas", "mirror_biting", "mirror_biting.bmp")
)
camera_parameters = utils.load_camera_params(
    os.path.join("data", "arenas", "mirror_biting", "mirror_biting.npz")
)
...
for prefix, data in main_data["run"].items():
    save_whole_plot(
        ...
        data["mask"],
        prefix,
        os.path.join("data", "mirror_biting", "output", prefix),
    )
    for arena in data["arenas"]:
        save_arena_plot(
            ...
            os.path.join("data", "mirror_biting", "output", f"{prefix}_{arena}"),
        )
for genotype, prefixes in main_data["by_genotype"].items():
    for prefix, data in prefixes.items():
        save_whole_plot(
            ...
            data["mask"],
            f"{prefix}: {genotype}",
            os.path.join("data", "mirror_biting", "output", f"{prefix}_{genotype}"),
        )
