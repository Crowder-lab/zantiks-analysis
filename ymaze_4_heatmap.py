...
for prefix, data in main_data["run"].items():
    save_whole_plot(
        ...
        data["mask"],
        prefix,
        os.path.join("data", "ymaze_4", "output", prefix),
    )
    for arena in data["arenas"]:
        save_arena_plot(
            ...
            os.path.join("data", "ymaze_4", "output", f"{prefix}_{arena}"),
        )
for genotype, prefixes in main_data["by_genotype"].items():
    for prefix, data in prefixes.items():
        save_whole_plot(
            ...
            data["mask"],
            f"{prefix}: {genotype}",
            os.path.join("data", "ymaze_4", "output", f"{prefix}_{genotype}"),
        )
