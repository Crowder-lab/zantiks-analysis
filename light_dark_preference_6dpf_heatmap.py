...
all_files = utils.find_heatmap_data("light_dark_preference_6dpf", suffixes)

# get files
main_files = all_files["main_files"]
wildtype_files = all_files["wildtype_files"]
arena_map = utils.load_arena_map(
    os.path.join("data", "arenas", "12_well", "12_well.bmp")
)
camera_parameters = utils.load_camera_params(
    os.path.join("data", "arenas", "12_well", "12_well.npz")
)
...
for prefix, data in main_data["run"].items():
    save_whole_plot(data["data"].copy(), data["mask"], prefix, os.path.join("data", "light_dark_preference_6dpf", "output", prefix))
    for arena in data["arenas"]:
        save_arena_plot(data["data"].copy(), arena, f"{prefix}: Arena {arena}", os.path.join("data", "light_dark_preference_6dpf", "output", f"{prefix}_{arena}"))
for genotype, prefixes in main_data["by_genotype"].items():
    for prefix, data in prefixes.items():
        save_whole_plot(data["data"].copy(), data["mask"], f"{prefix}: {genotype}", os.path.join("data", "light_dark_preference_6dpf", "output", f"{prefix}_{genotype}"))
