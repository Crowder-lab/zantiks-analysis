...
all_files <- find_data("distance_travelled", suffixes)
...
for (prefix_name in names(main_files)) {
  prism_data <- column_data(main_data[["distance"]][[prefix_name]], "DISTANCE")
  write_csv(prism_data, file.path("data", "distance_travelled", "output", paste0(prefix_name, "_DISTANCE.csv")))
}
...
prism_data <- column_data(for_prism, "DISTANCE")
write_csv(prism_data, file.path("data", "distance_travelled", "output", "combined_DISTANCE.csv"))
